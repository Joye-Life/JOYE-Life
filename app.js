...
function renderProfile(){
  const p=state.profile;
  const host=document.querySelector('#profile-summary');
  if(!p){host.innerHTML='<p>Complete setup to build your personal plan.</p>';return}
  const focusNames={money:'financial stability',career:'career momentum',health:'better health',organization:'a calmer, more organized life',business:'building a side business'};
  const energyCopy={low:'Your plan should favor shorter, lower-friction actions on most weekdays.',medium:'Your workload can flex between focused sessions and lighter maintenance tasks.',high:'You can support deeper work blocks, but Joye will still protect recovery and avoid overload.'};
  const payCopy={weekly:'weekly',biweekly:'every two weeks',semimonthly:'twice a month',monthly:'monthly'};
  const direction=p.careerGoal&&p.currentRole?`You are building from ${p.currentRole} toward ${p.careerGoal}.`:(p.careerGoal?`Your career direction is ${p.careerGoal}.`:'Your career direction is still open, so Joye will prioritize broadly useful progress.');
  const outcome=p.ninetyDayGoal?`Over the next 90 days, your plan should turn that outcome into smaller checkpoints instead of repeating it as one large task.`:'Your 90-day outcome still needs definition, so Joye will focus on short-term clarity first.';
  const pressure=p.stressPoint?'You also identified an active source of pressure. Joye will favor actions that reduce uncertainty and prevent it from competing with every other priority.':'No major pressure point is saved, so recommendations will rely more heavily on deadlines, goals, and daily capacity.';
  host.innerHTML=`<div class="plan-summary-hero"><span class="eyebrow">JOYE SUMMARY</span><h3>Your plan is centered on ${esc(focusNames[p.primaryFocus]||'steady progress')}.</h3><p>${esc(direction)} ${esc(outcome)}</p></div>
  <div class="plan-summary-grid">
    <article><small>How Joye will pace you</small><strong>${esc(energyCopy[p.energyLevel]||energyCopy.medium)}</strong></article>
    <article><small>Your planning rhythm</small><strong>About ${Number(p.availableTime||60)} minutes on a typical weekday, with money reviewed ${esc(payCopy[p.payFrequency]||'on each payday')}.</strong></article>
    <article><small>What Joye will protect</small><strong>${esc(pressure)}</strong></article>
  </div>
  <div class="plan-priorities"><span class="eyebrow">WHAT THIS MEANS</span><ul><li>Recommend actions that fit your actual time and energy.</li><li>Connect daily work to your strongest focus instead of treating every task equally.</li><li>Explain tradeoffs when money, career, goals, or deadlines compete.</li></ul></div>`;
}
...
function sectionCard(title,body,steps,question,section){
  return `<article class="insight-card"><div><span class="eyebrow">JOYE INSIGHT</span><h3>${esc(title)}</h3><p>${esc(body)}</p></div><div class="insight-actions">${steps.map(x=>`<span>→ ${esc(x)}</span>`).join('')}</div><button class="secondary-button section-ask" data-section="${esc(section)}" data-section-question="${esc(question)}">Ask Joye about this</button><div class="section-coach" hidden><div class="section-coach-response">Ask a follow-up without leaving this section.</div><textarea rows="3"></textarea><button type="button" class="primary-button section-coach-send">Ask Joye</button></div></article>`;
}
...
document.querySelector('#milestone-form').onsubmit=e=>{e.preventDefault();const input=document.querySelector('#milestone-title');const title=input.value.trim();if(!title)return;state.milestones.push({id:Date.now(),title,done:false});input.value='';save();renderAll();requestRecommendation()};
...
function renderCoachAnswer(data, box=document.querySelector('#coach-response')) {
...
async function askJoye(question, box=document.querySelector('#coach-response'), button=document.querySelector('#ask-coach')) {
  button.disabled=true;
  box.innerHTML='<div class="coach-thinking">Joye is reading your plan and thinking about your question…</div>';
  try {
    const response=await fetch('/api/coach',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question,profile:state.profile,money:state.money,tasks:state.tasks,goals:state.goals,milestones:state.milestones,weeklyPlan:state.weeklyPlan,dailyCheckIn:state.dailyCheckIn,history:state.coachHistory||[]})});
...
const onboarding=document.querySelector('#onboarding-modal');let step=1;function showStep(n){step=n;document.querySelectorAll('.onboarding-step').forEach(s=>s.classList.toggle('active',Number(s.dataset.step)===n));document.querySelector('#step-label').textContent=`Step ${n} of 4`;document.querySelector('#onboarding-progress-bar').style.width=`${n*25}%`;const back=document.querySelector('#onboarding-back'),next=document.querySelector('#onboarding-next'),finish=document.querySelector('#onboarding-finish');back.hidden=n===1;next.hidden=n===4;finish.hidden=n!==4;next.disabled=n===4;finish.disabled=n!==4}
